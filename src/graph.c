...
void dfs_rec(Graph *graph, uint **adj_list, int *deg, int *idx, int start, int visited[],
  ...
  for (int i = 0; i < deg[start]; i++) {
    ...
    // Jeśli sąsiad nie został jeszcze odwiedzony, to staje się on naszym nowym wierzchołkiem
    // startowym w kolejnym wywołaniu DFS.
    if (visited[neighbor] == 0)
      dfs_rec(graph, adj_list, deg, idx, i, visited, dfs_res);
  }
}
