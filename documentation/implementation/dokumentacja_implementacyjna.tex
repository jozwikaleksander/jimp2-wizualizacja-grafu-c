...
\textcolor{red}{(*)}Funkcja słuzy wyłącznie dla diagnostyki. Jej wywołanie jest zakomentowane w kodzie.
...
Pierwszy i ostatni element zawsze będą takie same, ponieważ jest to prezentacja zamknętego cyklu.\\ 
\textbf{Argumenty:}
\begin{itemize}
...
Funkcja służy do zwolnienia pamięci zaalokowanej dla struktury listy sąsiedztwa.
\begin{mylisting}
...
\end{mylisting}
W pierwszej części funkcja alokuje pamięć dla listy sąsiedztwa oraz pomocniczą tablicę \texttt{new\_pos}, która przechowuje nowo obliczone współrzędne przed ich ostatecznym zapisaniem do struktury grafu.
...
\end{mylisting}
Następnie graf jest analizowany w celu znalezienia cyklu (poligonu zewnętrznego). Wierzchołki należące do tego cyklu są oznaczane jako \texttt{fixed} (stałe) i rozmieszczane na okręgu za pomocą funkcji \texttt{place\_on\_circle}.
...
//Wypisywanie pozycji wierzchołków po rozstawianiu wiezchołków poligonu zewnętrznego
    ...
    while(t < max_iterations && max_change > MINIMUM_CHANGE) {
        ...
        //4.2.Iteracja po każdej parze nowych koordynat dla wierzchołków, jeśli one mają tę samą pozycję, to rozsuwamy je
                for (int a = 0;a<n; a++ ){
                    for(int b =a+1;b<n;b++){
